//gameScript loads x01DartsGame object from session storage and allows players to play match

//import JS game object
import {x01DartsGame} from "/assets/data/gameData.js"

//target undo buttons at start as used in x01DartsGame object
...
function loadGameData() {
    const storedGameData = sessionStorage.getItem('x01GameData');
    let parsedGameData = JSON.parse(storedGameData)

    //load game data into darts object
    x01DartsGame._game[0] = parsedGameData._game[0];
    x01DartsGame._game[1] = parsedGameData._game[1];

    //load player data into darts object
    x01DartsGame._players[0] = parsedGameData._players[0];
    x01DartsGame._players[1] = parsedGameData._players[1];

    //load current player data
    x01DartsGame._currentPlayer = parsedGameData._currentPlayer;
    ...
    if(x01DartsGame._currentPlayer === 1) {
...
function updateScoreDisplayP1(enteredScoreP1) {
    ...
    console.log(`Updated score to ${x01DartsGame._game[0].remainingScore}`)
}

function updateScoreDisplayP2(enteredScoreP2) {
    const gameScoreP2 = document.getElementById("gameScoreP2");
    gameScoreP2.innerText = x01DartsGame._game[1].remainingScore;
    console.log(`Updated score to ${x01DartsGame._game[1].remainingScore}`)
}

function showHistoryP1() {
    if(x01DartsGame._scoreHistory[0].scores.length !== 0) {
    const historyP1 = document.getElementById("scoreHistoryP1");
    historyP1.innerText = x01DartsGame._scoreHistory[0].scores[x01DartsGame._scoreHistory[0].scores.length - 1];
    } else {
    const historyP1 = document.getElementById("scoreHistoryP1");
...
function showHistoryP2() {
    if(x01DartsGame._scoreHistory[1].scores.length !== 0) {
        const historyP2 = document.getElementById("scoreHistoryP2");
        historyP2.innerText = x01DartsGame._scoreHistory[1].scores[x01DartsGame._scoreHistory[1].scores.length - 1];
        } else {
        const historyP2 = document.getElementById("scoreHistoryP2");
...
function updateStatsP1() {
    ...
    dartsThrown.innerText = x01DartsGame._gameStats[0].dartsThrown; //show darts thrown
    dartsAvg.innerText = x01DartsGame._gameStats[0].scoreAvg; //show average score
    console.log(`Updated darts thrown to ${x01DartsGame._gameStats[0].dartsThrown}`);
    console.log(`Updated average score to ${x01DartsGame._gameStats[0].scoreAvg}`);
}
...
function updateStatsP2() {
    ...
    dartsThrown.innerText = x01DartsGame._gameStats[1].dartsThrown; //show darts thrown
    dartsAvg.innerText = x01DartsGame._gameStats[1].scoreAvg; //show average score
    console.log(`Updated darts thrown to ${x01DartsGame._gameStats[1].dartsThrown}`);
    console.log(`Updated average score to ${x01DartsGame._gameStats[1].scoreAvg}`);
}

function showFinishP1() {
    const finishDisplayP1 = document.getElementById("finishDisplayP1");
    const currentFinishP1 = isOnAFinish(0);

    if (x01DartsGame._game[0].remainingScore <= 170) {
...
function showFinishP2() {
    ...
    const currentFinishP2 = isOnAFinish(1);

    if (x01DartsGame._game[1].remainingScore <= 170) {
...
function nextLeg() {
    ...
    if(x01DartsGame._game[0].legsToWin !== 0 || x01DartsGame._game[1].legsToWin !== 0) {
        let legsWonP1 = document.getElementById("legsWonP1");
        let legsWonP2 = document.getElementById("legsWonP2");
        legsWonP1.innerText = x01DartsGame._game[0].legsWon;
        legsWonP2.innerText = x01DartsGame._game[1].legsWon;
    }
};

function returnFullName(player) {
        //returns full name of player
        return `${x01DartsGame._players[player].firstName} ${x01DartsGame._players[player].dartsName} ${x01DartsGame._players[player].lastName}`
    };

 function determineNextPlayer(player) {
        if(player === 0) {
            x01DartsGame._currentPlayer = 1 //swaps to net player
            submitButtonP1.className = "btn bg-secondary bg-gradient" //changes class
            submitButtonP2.className = "btn bg-success bg-gradient" //changes class
...
function setScore(score, player) {
        ...
        if((x01DartsGame._game[player].remainingScore = x01DartsGame._game[player].remainingScore - score) === 0) {
            x01DartsGame._scoreHistory[player].scores.push(score) //update score history array
            x01DartsGame._gameStats[player].cumScore = score += x01DartsGame._gameStats[player].cumScore //update cumulative score
            x01DartsGame._gameStats[player].visits += 1 //increments visits
            x01DartsGame._gameStats[player].dartsThrown = x01DartsGame._gameStats[player].dartsThrown +=3 //calculates darts thrown. assumes to be 3 every throw.
            x01DartsGame._gameStats[player].scoreAvg = Math.round(x01DartsGame._gameStats[player].cumScore / x01DartsGame._gameStats[player].visits) //calculates average score
            hasWonLeg(player);
            hasWonGame(player);
        } else {
            x01DartsGame._scoreHistory[player].scores.push(score) //update score history array
            ...
            x01DartsGame._gameStats[player].dartsThrown = x01DartsGame._gameStats[player].dartsThrown +=3 //calculates darts thrown. assumes to be 3 every throw.
            x01DartsGame._gameStats[player].scoreAvg = Math.round(x01DartsGame._gameStats[player].cumScore / x01DartsGame._gameStats[player].visits) //calculates average score
            determineNextPlayer(player) //set next player
            console.log(`Player ${x01DartsGame._currentPlayer + 1} is up next.`)
        }    
    };

function deleteScore(player) {
        //delete last score from score history
        x01DartsGame._game[player].remainingScore = x01DartsGame._game[player].remainingScore + x01DartsGame._scoreHistory[player].scores[x01DartsGame._scoreHistory[player].scores.length - 1] //add last score back on
        x01DartsGame._gameStats[player].cumScore = x01DartsGame._gameStats[player].cumScore - x01DartsGame._scoreHistory[player].scores[x01DartsGame._scoreHistory[player].scores.length - 1] //remove last score from cum score for avg recalculation
        x01DartsGame._scoreHistory[player].scores.pop() //remove last score from history

        if(x01DartsGame._gameStats[player].dartsThrown !== 0) {
            x01DartsGame._gameStats[player].dartsThrown = x01DartsGame._gameStats[player].dartsThrown - 3 //remove darts thrown and avoid negative numbers
        } 

        if(x01DartsGame._gameStats[player].visits > 1) {
            x01DartsGame._gameStats[player].visits = x01DartsGame._gameStats[player].visits - 1 //remove last visit
            x01DartsGame._gameStats[player].scoreAvg = Math.round(x01DartsGame._gameStats[player].cumScore / x01DartsGame._gameStats[player].visits) //calculates average score
        } else {
...
function isOnAFinish(player){
        ...
        if(x01DartsGame._game[player].remainingScore === 0) {
            return "Well done!"
        }

        if(x01DartsGame._game[player].remainingScore <= 170) {
            x01DartsGame._game[player].canFinish = true;
            return dartsFinishes[x01DartsGame._game[player].remainingScore];

        } else {
            x01DartsGame._game[player].canFinish = false
        }
    }

function hasWonLeg(player){
        ...
        if(x01DartsGame._game[player].remainingScore === 0) {
            ...
            x01DartsGame._game[player].legsWon += 1;
            determineNextPlayer(player) //BUG this overrrides buttons

            alert(`Well done! ${returnFullName(player)} wins this leg! Click 'Next Leg' button to proceed.`);
        }
    }

function hasWonGame(player){
        ...
        if(x01DartsGame._game[player].legsWon === x01DartsGame._game[player].legsToWin) {
            submitButtonP1.className = "btn bg-secondary bg-gradient" 
            submitButtonP2.className = "btn bg-secondary bg-gradient"
            submitButtonP1.disabled = true;
            ...
            x01DartsGame._game[player].matchesWon += 1
            matchesWon.innerText = x01DartsGame._game[player].matchesWon
            newMatch.className = "btn bg-success bg-gradient";
            console.log(`${returnFullName(player)} has won ${x01DartsGame._game[player].legsWon} of ${x01DartsGame._game[player].totalLegs}`);
            alert(`Well done! ${returnFullName(player)} wins the game!!`);
        }
    };
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    nextLegButton.addEventListener("click", function() {
        x01DartsGame._scoreHistory = [{scores: []}, {scores: []}] //reset score history
        x01DartsGame._game[0].remainingScore = 501; //reset score
        x01DartsGame._game[1].remainingScore = 501; //reset score
        x01DartsGame._game[0].canFinish = false; //reset finish
        x01DartsGame._game[1].canFinish = false; //reset finish
        x01DartsGame._gameStats[0].dartsThrown = 0; //reset darts thrown (but keep average & total score)
        x01DartsGame._gameStats[1].dartsThrown = 0;//reset darts thrown (but keep average & total score)
        x01DartsGame._currentPlayer === 0; //reset player
    ...
    submitButtonP1.addEventListener("click", function() {
        ...
        } else if (enteredScoreP1 > x01DartsGame._game[0].remainingScore || x01DartsGame._game[0].remainingScore - enteredScoreP1 === 1) {
            alert("Invalid score.")
        } else if (!isNaN(enteredScoreP1)) {
            setScore(enteredScoreP1, 0); //trigger the scoring function
    ...
    submitButtonP2.addEventListener("click", function() {
        ...
        if (enteredScoreP2 > 180) {
            alert("Score higher than maximum allowed.")
        } else if (enteredScoreP2 > x01DartsGame._game[1].remainingScore || x01DartsGame._game[1].remainingScore - enteredScoreP2 === 1) {
            alert("Invalid score.")
        } else if (!isNaN(enteredScoreP2)) {
            setScore(enteredScoreP2, 1); //trigger the scoring function
    ...
    //delete score buttons update x01DartsGame object, bring the go back to the player and refresh HTML elements to show removed score
    deleteButtonP1.addEventListener("click", function() {
        if(x01DartsGame._game[0].startingScore !== x01DartsGame._game[0].remainingScore){
        deleteScore(0)
        determineNextPlayer(x01DartsGame._currentPlayer)
        updateStatsP1()
        updateScoreDisplayP1()
        showFinishP1()
        showHistoryP1()
    ...
    deleteButtonP2.addEventListener("click", function() {
        if(x01DartsGame._game[1].startingScore !== x01DartsGame._game[1].remainingScore){
        deleteScore(1)
        determineNextPlayer(x01DartsGame._currentPlayer)
        updateStatsP2()
        updateScoreDisplayP2()
